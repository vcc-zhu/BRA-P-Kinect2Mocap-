...
guass_mean1_path='./Kinect2mocap_perceptualautoencoder_layer512_keepprob80/b0.0005s0.0002'
#读取数据
X_origi= np.load('./train_data/rotate_mocap_120_train_August_shuffle_RootFix.npz')['X']#(3n,120,73)
X_noise = np.load('./train_data/rotate_mocap_120_train_August_shuffle_RootFix.npz')['X']
...
#        e1 = fc(X,n_inputs, n_encoder_fc1,name='e1')#(?, 128)
        e2 = fc(X,n_inputs, n_encoder_fc2,name='e2') #(?, 256) 
...
position_loss=0.5 * tf.reduce_mean(tf.pow(tf.subtract(recon,y),2.0))
...
loss=position_loss+0.0002*loss_smooth+0.0005*loss_bone_longeth
...
# =============================================================================
    saver.restore(sess, './Kinect2mocap_perceptualautoencoder_layer512_keepprob80/b0s0/model/model.ckpt-100')
#        
#     init_op = tf.global_variables_initializer() 
#     sess.run(init_op)
#     #加载训练好的参数parameter
#     variables = slim.get_variables_to_restore()
#     variables_to_restore_CMU_parameter = [v for v in variables if 'CMU_AutoEncoder' in v.name]
...
# =============================================================================
#     saver_parameter_CMU.restore(sess, './cmu_AutoEncoder_model/model.ckpt-200')
# =============================================================================
     
    last_mean_loss = 0
    last_mean_loss_smooth = 0
    last_mean_loss_bone = 0
    last_mean_loss_position = 0

    
    for epoch in range(300):
        epochtime1=time.time()
        last_mean_loss_list = []
        last_mean_loss_position_list = []
        last_mean_loss_smooth_list = []
        last_mean_loss_bone_list = []
       
        
        for each in range(int(data_file_number/batch_size)):#每个epoch
            ls,_,Xnoise,Xrecon,Xlabel,position_loss_value,smooth_loss_value,bone_loss_value= sess.run([loss,train_op,x,recon,y,position_loss,loss_smooth,loss_bone_longeth], feed_dict={x:X_noise[each*batch_size:(each+1)*batch_size,:,:],y:X_origi[each*batch_size:(each+1)*batch_size,:,:]})
            
            
            last_mean_loss_list.append([ls])
            last_mean_loss_position_list.append([position_loss_value])
            last_mean_loss_smooth_list.append([smooth_loss_value])
            last_mean_loss_bone_list.append([bone_loss_value])

            if (each%50==0):
                print('epoch:',epoch,'each:',each,'loss:',ls,'position_loss',position_loss_value,'smooth_loss:',smooth_loss_value,'bone_loss:',bone_loss_value)
                ...
                savedata(Xlabel[0],epoch,each,3)
#                
        #        savedata_loss(losslist,epoch)
        
        curr_mean = np.mean(last_mean_loss_list)
        curr_mean_position = np.mean(last_mean_loss_position_list)
        ...
        diff_mean_position, last_mean_loss_position = curr_mean_position-last_mean_loss_position, curr_mean_position
        ...
        print('curr_mean_position:',curr_mean_position,'diff_mean_position：',diff_mean_position)
    
        f1 = open(guass_mean1_path+'/mean_loss.txt','a')
        f2 = open(guass_mean1_path+'/mean_loss_smooth.txt','a')
        f3 = open(guass_mean1_path+'/mean_loss_bone.txt','a')
        f4 = open(guass_mean1_path+'/mean_loss_position.txt','a')
        ...
        f2.write(curr_mean_smooth_w+'\n')
        f2.close()

        curr_mean_bone_w=str(curr_mean_bone)
        f3.write(curr_mean_bone_w+'\n')
        f3.close()

        curr_mean_position_w=str(curr_mean_position)
        f4.write(curr_mean_position_w+'\n')
        f4.close()
